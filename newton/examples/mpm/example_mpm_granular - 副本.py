...
class Example:
    def __init__(self, viewer, options):
        ...
        mpm_options.support_radius = 0.15
        mpm_options.viscosity = 0.01
        mpm_options.fluid_density = 1000.0
        mpm_options.bulk_modulus = 2.0e6
    ...
    def test(self):
        ...
        cube_extents = CUBE_FULL_EXTENTS
...
def _create_collider_mesh(collider: str):
    ...
    if collider == "cube":
        cube_points, cube_indices = newton.utils.create_box_mesh(extents=CUBE_FULL_EXTENTS)

        return wp.Mesh(
            wp.array(cube_points[:, 0:3] + CUBE_CENTER, dtype=wp.vec3),
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--collider", default="none", choices=["cube", "wedge", "none"], type=str)
